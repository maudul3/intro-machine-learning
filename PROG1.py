...
class TwoLayerNeuralNetwork:
    def __init__(self, train_x, train_y, test_x, test_y, learning_rate, momentum, hidden_units):
        self.train_x = train_x
        self.train_y = train_y
        self.test_x = test_x
        self.test_y = test_y
        self.hidden_layer = np.size()
        self.learning_rate = learning_rate
        self.momentum = 1


def forward_propagate(input_layer, IL_to_HL_weights, HL_to_OL_weights):
    """
    Args:
        input_layer ():
        IL_to_HL_weights ():
    ...
    hidden_layer = np.array([sigmoid(val) for val in np.matmul(IL_to_HL_weights, input_layer)])
...
def backward_propagate_outer_layer(
        learning_rate, momentum, hidden_layer, HL_to_OL_weights, OL_error, previous_weight_delta
):
    """
    ...
    HL_to_OL_weight_delta = (
        learning_rate*np.outer(hidden_layer, OL_error) + momentum*previous_weight_delta
    )

    # Update Hidden Layer Weights
    return (HL_to_OL_weights + HL_to_OL_weight_delta.T), HL_to_OL_weight_delta

def backward_propagate_hidden_layer(
    learning_rate, 
    input_vector, 
    hidden_layer_error, 
    momentum, 
    previous_IL_to_HL_weight_delta,
    IL_to_HL_weights
):
    #Backpropagate for HL --> IL
    # Remove bias from HL_error vector as it has no connection to input layer
    IL_to_HL_weight_delta = (
        learning_rate*np.outer(input_vector, hidden_layer_error[1::]) + momentum*previous_IL_to_HL_weight_delta
    )

    return (IL_to_HL_weights+IL_to_HL_weight_delta.T), IL_to_HL_weight_delta
'''
def sum_jth_weight_for_all_k(weights, output_error):
    """
